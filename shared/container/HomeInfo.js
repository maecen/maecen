...
export default () => {
  let rows = ['band','masonry'].map((rowName) => {
    let blocks = textBlocks[rowName].map((cell) => {
      return (<TextBlock title={cell.title} description={cell.description}/>)
    })

    return (<Row style={style[rowName]}> {blocks} </Row>)
  })
